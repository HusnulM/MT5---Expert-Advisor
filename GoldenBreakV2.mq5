...
input group "=== Pullback Entry (BARU) ==="
input bool   UseWaitForPullback      = false; // true = tunggu pullback dulu sebelum entry, bukan langsung entry pas breakout
input double PullbackRetracePercent  = 30.0;  // Minimal % pullback dari jarak impuls breakout sebelum dipertimbangkan entry
input int    MaxBarsWaitPullback     = 10;    // Batalkan sinyal kalau pullback tidak terjadi dalam segini bar
input bool   RequireResumeConfirmation = true; // Butuh 1 candle tanda resume ke arah trend sebelum entry (bukan cuma sekadar pullback doang)
...
// BARU: state machine buat fitur "tunggu pullback sebelum entry"
enum ENUM_BREAKOUT_STATE { BSTATE_IDLE, BSTATE_WAIT_PULLBACK_BUY, BSTATE_WAIT_PULLBACK_SELL };
ENUM_BREAKOUT_STATE breakoutState = BSTATE_IDLE;
double waitBreakoutLevel  = 0;  // level highest/lowest (TANPA buffer) yang ditembus
double waitImpulseExtreme = 0;  // titik ekstrem (highest high / lowest low) sejak breakout terjadi
int    waitBarsElapsed    = 0;
...
void DB_Update()
{
   ...
   // BARU: override status kalau lagi dalam mode nunggu pullback
   if(breakoutState != BSTATE_IDLE)
   {
      statusText = StringFormat("Status  : WAIT PULLBACK %s (bar %d/%d)",
                                 (breakoutState==BSTATE_WAIT_PULLBACK_BUY?"BUY":"SELL"),
                                 waitBarsElapsed, MaxBarsWaitPullback);
      statusClr = clrYellow;
   }
...
double CalculateLotSize(double slDistancePrice)
{
   ...
   // FIX BUG FATAL: SYMBOL_TRADE_TICK_VALUE/TICK_SIZE terbukti TIDAK AKURAT
   // untuk instrumen CFD (gold, index, dll) -- ini limitasi resmi MQL5/
   // MetaQuotes, tickValue reliable buat forex biasa (SYMBOL_CALC_MODE_FOREX)
   // tapi TIDAK utk CFD (SYMBOL_CALC_MODE_CFD) di mana value-nya tergantung
   // harga saat ini. Terbukti dari debug: perhitungan lama ngasih valuePerLot
   // yang 10x LEBIH KECIL dari floating P&L REAL yang di-track MT5 sendiri --
   // artinya posisi yang dibuka 10x LEBIH BESAR dari niat risk cap.
   //
   // Fix: pakai SYMBOL_TRADE_CONTRACT_SIZE langsung (contract size asli,
   // misal 100oz gold), dikali jarak harga -- ini pendekatan yang
   // direkomendasikan resmi buat CFD, nggak bergantung ke tickValue yang
   // nggak reliable.
   double contractSize = SymbolInfoDouble(_Symbol, SYMBOL_TRADE_CONTRACT_SIZE);
   double tickValue = SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_VALUE);   // disimpan cuma buat debug/perbandingan
   double tickSize  = SymbolInfoDouble(_Symbol, SYMBOL_TRADE_TICK_SIZE);    // disimpan cuma buat debug/perbandingan
   if(contractSize <= 0)
   {
      Print("ERROR: SYMBOL_TRADE_CONTRACT_SIZE invalid untuk ", _Symbol);
      return 0;
   }

   // Berapa "value" per 1.0 lot untuk jarak SL yang dihitung (FIX: pakai contract size)
   double valuePerLot = slDistancePrice * contractSize;
   if(valuePerLot <= 0) return 0;

   double lot = riskMoney / valuePerLot;

   // BARU (debug): print PERBANDINGAN kedua metode -- valuePerLot_OLD (pakai
   // tickValue/tickSize, TERBUKTI SALAH) vs valuePerLot_NEW (pakai contract
   // size, FIX). Biar kelihatan jelas selisihnya & bisa diverifikasi lagi.
   double valuePerLotOld = (tickSize > 0) ? (slDistancePrice / tickSize) * tickValue : 0;
   PrintFormat("DEBUG LotCalc: riskMoney=%.2f slDistPrice=%.5f contractSize=%.2f valuePerLot_NEW=%.5f rawLot_NEW=%.5f | (lama: tickValue=%.5f tickSize=%.5f valuePerLot_OLD=%.5f rawLot_OLD=%.5f)",
               riskMoney, slDistancePrice, contractSize, valuePerLot, lot,
               tickValue, tickSize, valuePerLotOld, (valuePerLotOld>0? riskMoney/valuePerLotOld : 0));
...
//+------------------------------------------------------------------+
//| BARU: eksekusi entry sebenarnya -- dipakai baik dari jalur         |
//| langsung (UseWaitForPullback=false) maupun jalur pullback         |
//| terkonfirmasi (UseWaitForPullback=true). SL/TP/lot dihitung FRESH  |
//| pakai ATR & harga SAAT INI, bukan dari saat breakout awal terjadi  |
//| (karena kalau lewat jalur pullback, udah beberapa bar berlalu).    |
//+------------------------------------------------------------------+
void ExecuteEntry(bool wantBuy)
{
   // Re-validasi filter dasar di titik eksekusi -- market bisa berubah
   // kondisi selama masa tunggu pullback, jadi dicek ulang bukan cuma
   // dipercaya dari kondisi pas breakout awal terdeteksi.
   string newsName;
   if(IsNewsBlackout(newsName))
   {
      Print("Skip entry (pullback confirmed): news blackout aktif -- ", newsName);
      return;
   }

   double adx, atr;
   if(!GetADX(adx) || !GetATR(atr)) return;
   if(adx < ADXThreshold)
   {
      Print("Skip entry (pullback confirmed): ADX sudah turun di bawah threshold (", adx, "), trend melemah selama masa tunggu.");
      return;
   }

   if(wantBuy && !AllowLong)  { Print("Entry BUY dibatalkan: AllowLong=false."); return; }
   if(!wantBuy && !AllowShort){ Print("Entry SELL dibatalkan: AllowShort=false."); return; }
...
//+------------------------------------------------------------------+
//| Cek sinyal breakout awal. Kalau UseWaitForPullback=false, langsung |
//| entry sesuai perilaku lama. Kalau true, JANGAN entry -- masuk ke   |
//| state "nunggu pullback" dulu (lihat ManagePullbackWaiting()).      |
//+------------------------------------------------------------------+
void CheckEntrySignal()
{
   ...
   if(closePrice > highest + buffer)
   {
      ...
      if(!UseWaitForPullback)
      {
         ExecuteEntry(true);
         return;
      }

      // BARU: masuk mode nunggu pullback, JANGAN entry sekarang
      breakoutState      = BSTATE_WAIT_PULLBACK_BUY;
      waitBreakoutLevel  = highest;
      waitImpulseExtreme = iHigh(_Symbol, PERIOD_CURRENT, 1);
      waitBarsElapsed    = 0;
      Print("Breakout BUY terdeteksi di ", closePrice, " -- MENUNGGU PULLBACK sebelum entry (level=", highest, ").");
      return;
   }
   ...
   if(closePrice < lowest - buffer)
   {
      ...
      breakoutState      = BSTATE_WAIT_PULLBACK_SELL;
      waitBreakoutLevel  = lowest;
      waitImpulseExtreme = iLow(_Symbol, PERIOD_CURRENT, 1);
      waitBarsElapsed    = 0;
      Print("Breakout SELL terdeteksi di ", closePrice, " -- MENUNGGU PULLBACK sebelum entry (level=", lowest, ").");
      return;
   }
}

//+------------------------------------------------------------------+
//| BARU: kelola state "nunggu pullback" -- dipanggil tiap bar baru    |
//| selama breakoutState != IDLE. Entry baru dieksekusi di sini kalau  |
//| pullback + konfirmasi resume terpenuhi.                            |
//+------------------------------------------------------------------+
void ManagePullbackWaiting()
{
   if(breakoutState == BSTATE_IDLE) return;

   bool wantBuy = (breakoutState == BSTATE_WAIT_PULLBACK_BUY);
   ...
   // Update titik ekstrem impuls (buat ngukur seberapa besar pullback-nya nanti)
   if(wantBuy) waitImpulseExtreme = MathMax(waitImpulseExtreme, highNow);
   else        waitImpulseExtreme = MathMin(waitImpulseExtreme, lowNow);

   // Batalkan kalau kelamaan nunggu tanpa pullback yang cukup
   if(waitBarsElapsed > MaxBarsWaitPullback)
   {
      Print("Sinyal breakout ", (wantBuy?"BUY":"SELL"), " DIBATALKAN: pullback tidak terjadi dalam ", MaxBarsWaitPullback, " bar.");
      breakoutState = BSTATE_IDLE;
      return;
   }

   // Batalkan kalau breakout-nya gagal total (harga balik ngelewatin level breakout awal)
   if(wantBuy && closeNow < waitBreakoutLevel)
   {
      Print("Sinyal breakout BUY DIBATALKAN: harga balik di bawah level breakout (", waitBreakoutLevel, ").");
      breakoutState = BSTATE_IDLE;
      return;
   }
   if(!wantBuy && closeNow > waitBreakoutLevel)
   {
      Print("Sinyal breakout SELL DIBATALKAN: harga balik di atas level breakout (", waitBreakoutLevel, ").");
      breakoutState = BSTATE_IDLE;
      return;
   }

   // Hitung seberapa jauh sudah pullback, sebagai % dari jarak impuls breakout
   double impulseSize = MathAbs(waitImpulseExtreme - waitBreakoutLevel);
   if(impulseSize <= 0) return; // belum ada data impuls yang valid

   double retraceDist = wantBuy ? (waitImpulseExtreme - closeNow) : (closeNow - waitImpulseExtreme);
   double retracePct  = (retraceDist / impulseSize) * 100.0;

   if(retracePct < PullbackRetracePercent) return; // pullback belum cukup dalam

   // Opsional: butuh 1 candle tanda mulai resume ke arah trend lagi
   if(RequireResumeConfirmation)
   {
      double closePrev = iClose(_Symbol, PERIOD_CURRENT, 2);
      if(wantBuy && closeNow <= closePrev) return;  // belum ada tanda mulai naik lagi
      if(!wantBuy && closeNow >= closePrev) return; // belum ada tanda mulai turun lagi
   }

   // Semua syarat terpenuhi -> entry sekarang
   Print("Pullback ", DoubleToString(retracePct,1), "% terkonfirmasi -- eksekusi entry ", (wantBuy?"BUY":"SELL"), " sekarang.");
   ExecuteEntry(wantBuy);
   breakoutState = BSTATE_IDLE;
}
...
void OnTick()
{
   ...
   if(!HasOpenPosition())
   {
      // BARU: kalau lagi dalam mode nunggu pullback, prioritaskan itu dulu
      // (jangan cari sinyal breakout baru sampai state ini selesai/batal).
      if(breakoutState != BSTATE_IDLE)
         ManagePullbackWaiting();
      else
         CheckEntrySignal();
   }
